...
see [Istio Installation](ISTIO_INSTALLATION.md) document.
...
```
Edit [MAPL_adapter_dep.yaml](insert_link) with the location of the adapter's docker image and imagePullSecrets.
...
Make sure [MAPL_adapter_dep.yaml](insert_link) has an imagePullSecrets section with docker-secret for the docker-registry field, and that the image can be pulled.
...
Update file [MAPL_adapter_dep.yaml](insert_link) with the following variables
...
The current set of rules [rules.yaml](insert_link) have the following effect:
