...
Upon "true" result, values may be returned from the data which the rule tests against. 

#### one-attribute-condition
A condition is defined as `<attribute, method, value>`  

* Attribute : a string from the [Supported Attributes](SUPPORTED_ATTRIBUTES.md).
  
* Method: a string from the [Supported Methods](SUPPORTED_METHODS.md).
...
conditions:
  ANY:
    ...
    condition:
      ...
      value: "abc"
```
**$VALUE** checks the json value.

example:

Check if a label with value “abc” exists (regardless of key):
```
conditions:
  ANY:
    parentJsonpathAttribute: "jsonpath:$.metadata.labels"
...
The returnValueJsonpath field is a []*map[string][]interface{} of jsonpath attributes that are returned for every sub-document which satisfies the node conditions.
...
remark: NOT Node - returns empty data
